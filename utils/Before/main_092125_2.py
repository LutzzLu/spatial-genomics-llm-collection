...
for i, one_paper in tqdm(enumerate(result)):
    ...
    # Run the fetch_paper_full_text.py script to get the full text
    import subprocess
    
    cmd = [
        "python", "fetch_paper_full_text.py",
        "--doi", doi
    ]
    ...
    try:
        ...
        if proc.returncode == 0:
            # Success - the script should have created files
            # Try to find and read the text file
            safe_doi = re.sub(r'[^\w\-_\.]', '_', doi)
            txt_path = Path(f"./Data/txt/{safe_doi}.txt")
            
            if txt_path.exists():
                full_text = txt_path.read_text(encoding="utf-8")
                print(f"Successfully fetched full text for DOI {doi}")
            else:
                full_text = None
                print(f"Text file not found for DOI {doi}")
        else:
            full_text = None
            print(f"Failed to fetch full text for DOI {doi}: {proc.stderr}")
            
    except subprocess.TimeoutExpired:
        full_text = None
        print(f"Timeout while fetching full text for DOI {doi}")
    except Exception as e:
        full_text = None
        print(f"Error while fetching full text for DOI {doi}: {e}")
    # info = fetch_via_worker(doi, oa_url)
